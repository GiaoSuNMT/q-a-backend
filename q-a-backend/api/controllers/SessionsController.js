...
module.exports = {
  create: (req, res) => {
    try {
      ...
      Sessions.create({
        ...
        if (session) {
          return res.ok({
            message: "Tạo phiên hỏi đáp thành công",
            session: session,
  ...
  getbyid: async (req, res) => {
    try {
      ...
      const token = req.cookies.access_token.split(" ")[1];
      const userInfo = jwToken.decoded(token); // Người dùng đang đăng nhập
      let session = await Sessions.findOne({ id: sessionId }).populate(
        "questions"
      );
      if (!session) {
        ...
        if (sessions.length === 0) {
          ...
          Questions.destroy({ sessionId: sessionId }).exec((err, questions) => {
            ...
            for (let index = 0; index < questions.length; index++) {
              Answers.destroy({ questionId: questions[index].id }).exec(
                (err, answers) => {
                  if (err) {
                    return res.serverError("Database error");
                  }
