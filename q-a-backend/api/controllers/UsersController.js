...
module.exports = {
  create: (req, res) => {
    try {
      ...
      if (validate.checkUser(username) === false) {
        return res.badRequest(
          "Username chứa 6-16 kí tự chỉ số và chữ cái không có kí tự đặc biệt!"
        );
      }
      if (validate.checkPassword(password) === false) {
        return res.badRequest(
          "Password gồm 6-16 kí tự và chứa ít nhất 1 số và 1 chữ cái!"
        );
      }
      ...
      if (validate.checkName(name) === false) {
        return res.badRequest("Tên chưa hợp lệ!");
      }
    ...
    try {
      ...
      Users.findOne({ id: userId }).exec((err, user) => {
        ...
        if (!user) {
          ...
          return res.badRequest(
            "Không thể thay đổi vai trò của người dùng này"
          );
        } else {
      ...
      Users.findOne({ username: username }, async (err, user) => {
        if (err) {
          ...
          if (comparePassword) {
            ...
            return res
              .cookie("access_token", "Bearer " + jwToken.issue(data), {
                maxAge: 6 * 600 * 1000,
                httpOnly: true
              })
              .ok(data);
